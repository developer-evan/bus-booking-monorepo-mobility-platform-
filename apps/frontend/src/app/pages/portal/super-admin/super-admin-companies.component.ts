...
@Component({
  ...
  imports: [ReactiveFormsModule, Button, InputText, Tag, DatePipe],
  template: `
      ...
      <section class="rounded-xl border border-neutral-200 bg-white dark:border-neutral-800 dark:bg-neutral-900">
        <div class="flex items-center justify-between border-b border-neutral-200 px-5 py-4 dark:border-neutral-800">
          <div>
            ...
            <p class="text-sm text-neutral-500 dark:text-neutral-400">{{ companies().length }} onboarded</p>
          </div>
          <p-button
            label="Add company"
            icon="pi pi-plus"
        ...
        @if (showCreateForm()) {
          ...
          >
            ...
            <input pInputText type="email" formControlName="email" class="w-full" placeholder="Company email" />
            <input pInputText formControlName="phone" class="w-full" placeholder="Company phone" />
            <input pInputText formControlName="slug" class="w-full" placeholder="Slug (optional)" />
            @if (createError()) {
            ...
            <div class="flex gap-2">
              ...
              />
              <p-button label="Cancel" [text]="true" severity="secondary" (onClick)="showCreateForm.set(false)" />
            </div>
          </form>
        }
      ...
      <section class="rounded-xl border border-neutral-200 bg-white p-5 dark:border-neutral-800 dark:bg-neutral-900 sm:p-6">
        @if (!selectedCompany()) {
          ...
          <div class="mt-8 border-t border-neutral-200 pt-6 dark:border-neutral-800">
            ...
            <form class="mt-4 grid gap-3" [formGroup]="inviteAdminForm" (ngSubmit)="inviteAdmin()">
              <input
                pInputText
              ...
              />
              <input pInputText formControlName="phone" class="w-full" placeholder="Admin phone" />
              @if (inviteError()) {
...
export class SuperAdminCompaniesComponent implements OnInit {
  ...
  protected createCompany(): void {
    ...
    this.companiesApi
      .create({ name, email, phone, slug: slug || undefined })
      .subscribe({
        next: (company) => {
          this.companies.update((items) =>
            [...items, company].sort((a, b) => a.name.localeCompare(b.name)),
          );
